...
# --- 3. ログイン・同期管理（ノンブロッキング） ---
if 'logged_in' not in st.session_state: st.session_state.logged_in = False
if 'staff_info' not in st.session_state: st.session_state.staff_info = None

# ブラウザの記憶を取得（これに依存してst.stopしないのが画面を出すコツ）
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("SYSTEM LOGIN", use_container_width=True):
                ...
                if res.data:
                    ...
                    # JavaScriptで書き込み
                    st.markdown(f"<script>localStorage.setItem('staff_id', '{u_id}'); localStorage.setItem('session_key', '{new_key}');</script>", unsafe_allow_html=True)
                    st.session_state.logged_in = True
                    st.session_state.staff_info = res.data[0]
                    st.rerun()
                else: st.error("ID / PW不一致")
    # 万が一の同期遅延のために2秒おきにリフレッシュを試みる
    if saved_id is None:
        st_autorefresh(interval=2000, limit=3, key="sync_trigger")
    st.stop()
...
if choice == "📋 本日の業務":
    ...
    overdue = [l for l in l_data if l['target_hour'] < now_jst.hour and l['status'] != "completed"]
    if not overdue: st.success("全て完了しています！")
    else:
        for l in overdue:
            st.markdown("<div class='app-card'>", unsafe_allow_html=True)
            ca, cb = st.columns([3, 1])
            ca.write(f"**【遅延】{l['target_hour']:02d}:{l['target_minute']:02d} - {l['location_name']}**")

...
if not st.session_state.logged_in:
    ...
    st_autorefresh(interval=3000, limit=3, key="sync_wait") # 呼び水
    st.stop()
...
def render_task_execution(task):
    ...
    if not st.session_state[v_key]:
        ...
        if ph_in and st.button("✅ 報告送信", type="primary", use_container_width=True):
            ...
            st.cache_data.clear(); del st.session_state[v_key]; st.balloons(); st.rerun()

...
# --- 2. 究極のデザインCSS（PC版：高級カードデザイン / モバイル版：現場特化） ---
st.markdown("""
    <style>
    /* 全体共通：不要なパーツの完全抹殺 */
    ...
    /* PC版：高級感のある中央寄せカードデザイン (画面幅769px以上) */
...
if not st.session_state.logged_in:
    if saved_id is None: # JavaScriptからの返答待ち
        placeholder.caption("Connecting to System...")
    else:
        placeholder.empty()

    # タイトルを洗練
    st.markdown("<h1 style='text-align: center; color: #2c3e50; font-size: 28px; font-weight: 800; margin-bottom: 0;'>NATURAL STONE</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #7f8c8d; font-size: 14px; margin-bottom: 30px;'>OPERATION MANAGEMENT SYSTEM</p>", unsafe_allow_html=True)
...
choice = st.sidebar.radio("MENU", [m for m in menu_options if "Admin" not in m or staff['role'] == 'admin'], key="nav_radio")
...
if choice == "📋 本日の業務":
    ...
    if data.data:
        ...
        for r in data.data:
            ...
            df_l.append({"名前": r['staff_name'], "日付": r['work_date'], "出勤": c_in.strftime("%H:%M"), "退勤": c_out.strftime("%H:%M") if c_out else "未打刻", "実働時間": work_str})

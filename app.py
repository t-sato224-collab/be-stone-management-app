...
# CSS: サイドバー横幅75%・フォント特大(26px)・メニュー間隔(40px)
st.markdown("""
    <style>
    /* モバイルサイドバー横幅 75% 強制固定 */
    ...
    /* サイドバーメニュー：フォントをさらに大きく(26px)・間隔(40px) */
...
def render_task_execution(task):
    ...
    if not st.session_state[qr_v_key]:
        st.subheader("1. 現場QRを撮影（かざしてシャッター）")
        qr_in = st.camera_input("QRを枠に入れて撮影してください", key=f"qr_{task['id']}")
        if qr_in:
            if decode_qr(qr_in) == task['task_master']['locations']['qr_token']:
                st.session_state[qr_v_key] = True; st.rerun()
            else: st.error("場所が違います。正しい位置で撮影してください。")
    else:
        st.subheader("2. 完了後の写真を撮影")

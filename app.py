...
if not st.session_state.logged_in:
    if saved_id is None:
        st_autorefresh(interval=1000, limit=2, key="sync_init"); st.stop()
...
if choice == "📋 本日の業務":
    ...
    for r in res.data:
        ...
        # 00:00形式の変換
        h, m = divmod(act_m, 60)
        time_hm = f"{h:02d}:{m:02d}"
        time_desc = f"{h}時間{m}分"
        time_color = "red" if act_m >= 420 else "#000000"
        
        st.markdown(f"""
        **📅 {c_in.strftime('%Y年%m月%d日')}**  
        🕙 {c_in.strftime('%H:%M')} 〜 {c_out.strftime('%H:%M') if c_out else '勤務中'}  
        ✅ <span style='color:{time_color}; font-weight:bold;'>実働：{time_hm if c_out else '---'} ({time_desc if c_out else '---'})</span>  
        休憩：{br_m}分  
        ---
        """, unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if data.data:
        ...
        for r in data.data:
            ...
            df_l.append({
                ...
                "休憩(分)": br_m, "実働(00:00)": f"{h:02d}:{m:02d}", "実働(詳細)": f"{h}時間{m}分", "分": act_m
            })

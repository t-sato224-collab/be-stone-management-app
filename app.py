...
# --- C. メインエリア表示 ---
st.markdown("<h1 style='color: #75C9D7; margin-top: 0; margin-bottom: 0;'>BE STONE Pro</h1>", unsafe_allow_html=True)
    ...
    if data.data:
        ...
        for r in data.data:
            ...
            df_l.append({
                ...
                "休憩(分)": br_m, "実働(00:00)": f"{h:02d}:{m:02d}", "分": act_m
            })
        df = pd.DataFrame(df_l)
        # エラー修正：判定用の '分' は残したままスタイルを適用し、表示の直前で隠す
        def style_red(row): return ['color: red' if row['分'] >= 420 else '' for _ in row]
        styled_df = df.style.apply(style_red, axis=1).hide(axis='columns', subset=['分'])

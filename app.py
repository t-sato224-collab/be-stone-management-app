...
# --- 3. ログイン・セッション管理（ノンブロッキング） ---
if 'logged_in' not in st.session_state: st.session_state.logged_in = False
if 'staff_info' not in st.session_state: st.session_state.staff_info = None

# ブラウザの記憶を取得（これに依存してst.stopしないのがコツ）
...
if not st.session_state.logged_in:
    ...
    # ログインしていない間はここで止める。ただしJS読み込みを妨げない場所。
    st_autorefresh(interval=2000, limit=3, key="sync_wait")
    st.stop()
...
# 今日のタスクログ取得（VIEWを使用）
try:
    l_data = supabase.table("v_active_tasks_today").select("*").execute().data
    l_data = sorted(l_data, key=lambda x: (x['target_hour'] or 0, x['target_minute'] or 0))
except:
    st.error("データベースVIEWの読み込みに失敗しました。SQLを確認してください。")
    l_data = []
...
def render_task_execution(task):
    ...
    if st.button("⏸️ 中断してリストに戻る", use_container_width=True):
        supabase.table("task_logs").update({"status": "interrupted"}).eq("id", task['task_log_id']).execute(); st.rerun()
    st.divider()
    qr_v_key = f"qr_v_{task['task_log_id']}"
    if qr_v_key not in st.session_state: st.session_state[qr_v_key] = False
    qr_in = st.camera_input("1. 現場QRをスキャン", key=f"qr_{task['task_log_id']}")
    if qr_in:
        if decode_qr(qr_in) == task['qr_token']:
            st.session_state[qr_v_key] = True; st.success("QR確認成功！")
        else: st.error("場所が違います")
    if st.session_state[qr_v_key]:
        ph_in = st.camera_input("2. 完了写真撮影", key=f"ph_{task['task_log_id']}")
            ...
            for l in tasks_now[:5]:
                ...
                if l['status'] in ["pending", "interrupted"]:
                    if cb.button("着手" if l['status']=="pending" else "再開", key=f"s_{l['task_log_id']}", use_container_width=True):
                        supabase.table("task_logs").update({"status":"in_progress","staff_id":staff['id']}).eq("id",l['task_log_id']).execute(); st.rerun()
